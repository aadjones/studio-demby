...
function inferActivityCategories(a: MDXSketch): string[] {
  ...
  if (tags.some((t) => ["math", "theory", "paper"].includes(t))) {
    return ["provocations"];
  }
  if (tags.some((t) => ["game", "interactive", "tool"].includes(t))) {
    return ["systems-tools"];
  }
  if (tags.some((t) => ["satire", "absurd", "linguistics", "reference"].includes(t))) {
    return ["provocations"];
  }
